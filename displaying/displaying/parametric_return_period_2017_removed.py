...
da = da_orig.where(da_orig.time.dt.year != 2017, drop = True)
...
# plot the scatter
plt.scatter(tau[-1], u[-1], marker='o', facecolor='lightskyblue', edgecolor='blue', color='blue', alpha = 1, label = 'Non parametric Jan 2017 return period')
...
plt.axhline(da_orig.max(), lw=1, color='grey', ls='dotted', label='Jan 2017 value')
...
plt.savefig('../../../megafires_data/png/QN_parametric_return_period_2017_removed.png', dpi=300)
plt.show()
