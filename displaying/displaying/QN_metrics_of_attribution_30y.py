...
# get past period
da_cf = qn.sel(time=slice('1928','1957'))
...
normfit_cf = norm.fit(np_cf)
normfit_ac = norm.fit(np_ac)

# get ev value 
ev = qn.sel(time='2017')

# get return periods
tau_cf = 1/norm.sf(ev, *normfit_cf)
tau_ac = 1/norm.sf(ev, *normfit_ac)
...
# get delta
delta = ev - norm.isf(1/tau_ac, *normfit_cf)
...
for i in range(nboot):
    ...
    normfit_cf_i = norm.fit(z_cf_i)
    normfit_ac_i = norm.fit(z_ac_i)

    tau_cf_i = 1/norm.sf(ev, *normfit_cf_i)
    tau_ac_i = 1/norm.sf(ev, *normfit_ac_i)

    rr_ca_i = tau_cf_i/tau_ac_i
    far_ca_i = (tau_cf_i-tau_ac_i)/tau_cf_i
    delta_i = ev - norm.isf(1/tau_ac_i, *normfit_cf_i)
...
df.to_csv(f'../../../megafires_data/output/metrics_of_attr_QN_30y.csv')