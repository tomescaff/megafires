...
cr_mean = np.mean(np_jan_control_run)

# fit gev dist
gevfit_ar_1950_2021 = genextreme.fit(np_jan_all_runs_1950_2021)
gevfit_cr = genextreme.fit(np_jan_control_run)

# computing best x ticks
y = np.linspace(20, 34, 1000)
x = 1/genextreme.sf(y, *gevfit_ar_1950_2021)

# computing y values
y_gev_ar_1950_2021 = genextreme.isf(1/x, *gevfit_ar_1950_2021)
y_gev_cr = genextreme.isf(1/x, *gevfit_cr)
...
for i in range(nboot):
    ...
    gevfit_ = genextreme.fit(z)
    bspreds[i] = genextreme.isf(1/x, *gevfit_)
...
tau = 1/genextreme.sf(np.median(lens_ev), *gevfit_ar_1950_2021)
cr_ev = genextreme.isf(1/tau, *gevfit_cr)
...
plt.plot(x, y_gev_ar_1950_2021, color='b', lw=1.5, alpha = 1, label = 'Parametric return period ACTUAL ', zorder=4)
plt.plot(x, y_gev_cr, color='r', lw=1.5, alpha = 1, ls='--', label = 'Parametric return period CONTRAFACTUAL', zorder=4)
...
plt.axhline(np.median(lens_ev), lw=1, color='grey', ls='--', label='EE median value')

# plot the cr mean value
plt.axhline(cr_mean, lw=1, color='grey', ls='-.', label='Control run mean value')

# plot the cr mean value
plt.axhline(cr_ev, lw=1, color='grey', ls='-.')

# plot ee tau value
plt.axvline(tau, lw=1, color='k', ls='dotted', label='EE return period')
...
plt.savefig('../../../megafires_data/png/LENS_return_period_gev_with_fraction.png', dpi=300)
plt.show()