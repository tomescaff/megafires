...
# get LENS control run time series
basedir = '../../../megafires_data/LENS/'
...
past_ev = norm.isf(1/tau, *normfit_cr)
future_ev = norm.isf(1/tau, *normfit_ar_2070_2100)

tau_ee_cr = 1/norm.sf(temp_ee, *normfit_cr)
...
plt.gca().fill_between(x, yinf_cr, ysup_cr, color='b', alpha=.25)
...
plt.plot(x, y_norm_cr, color='b', lw=1.5, alpha = 1, label = 'Parametric return period PAST', zorder=4)
...
# plot the scatter
plt.scatter(tau_cr, u_cr, marker='o', facecolor='lightskyblue', edgecolor='none', color='blue', alpha = 0.5)
...
plt.axhline(np.mean(np_jan_control_run), lw=1, color='b', ls='-')
...
plt.axvline(tau_ee_cr, lw=1, color='b', ls='dotted')
...
plt.tight_layout()
plt.savefig('../../../megafires_data/png/LENS_return_period_CR_present_future_normal.png', dpi=300)
plt.show()
