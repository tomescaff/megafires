...
qn = stns.get_QN_tmax_dec_1911_2022().sel(time=slice('1911','2022'))
...
plt.plot(qn.sel(time='2022').time.dt.year, qn.sel(time='2022').values, lw=1, marker='o', markersize=7, markeredgecolor='blue', markerfacecolor='blue', color='blue', alpha=0.4)
...
plt.axhline(qn.sel(time='2022'), color='b', lw=1.5)#, ls='--')
ax.spines.right.set_visible(False)
ax.spines.top.set_visible(False)
ax.tick_params(direction="in")
for tick in ax.xaxis.get_major_ticks():
...
for tick in ax.yaxis.get_major_ticks():
    tick.label.set_fontsize(9) 
    tick.label.set_weight('light')
plt.bar([0], height=2022*slope + intercept-qn.mean().values, bottom=qn.mean().values, width=0.75, color='r', alpha=0.4)
plt.bar([1.35], height=qn.sel(time='2022').values-qn.mean().values, bottom=qn.mean().values, width=0.75, color='b', alpha=0.4)
plt.axhline(qn.mean().values,  lw=1.5, color='b', ls='--')
plt.xlim([-1.35, 2.7])
plt.xticks([0, 1.35])
plt.errorbar(x=0, y=2022*slope + intercept, yerr=delta[x==2022], lw=1.1, color='grey', capsize=4, fmt = '.', capthick=1.3, ecolor='grey', alpha=1)
...
for i in range(nboot):
    ...
    tau_1_i = 1/norm.sf(qn.sel(time='2022').values, *normfit_ac)
    tau_0_i = 1/norm.sf(qn.sel(time='2022').values, *normfit_cf)
...
normfit_ac = norm.fit(ac)
normfit_cf = norm.fit(cf)
tau_1 = 1/norm.sf(qn.sel(time='2022').values, *normfit_ac)
tau_0 = 1/norm.sf(qn.sel(time='2022').values, *normfit_cf)
rr_norm = tau_0/tau_1
...
for i in range(nboot):
    ...
    normfit_ac = norm.fit(ac)
    tau_i = 1/norm.sf(qn.sel(time='2022').values, *normfit_ac)
    tau_norm[i] = tau_i

tau_norm_inf, tau_norm_sup = np.quantile(tau_norm, [0.025, 0.975], axis = 0)
ac = bootstrap(qn.sel(time=slice('1928','2021')).values)
normfit_ac = norm.fit(ac)
tau = 1/norm.sf(qn.sel(time='2022').values, *normfit_ac)