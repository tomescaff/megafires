...
obs_mle_1880 = pd.read_csv(join(relpath, 'metrics_QN_MLE_1880_2022_normfit_1930_2022_dec_2022.csv'), index_col=0)
...
lens2_mle_1880 = pd.read_csv(join(relpath, 'metrics_LENS2_MLE_1880_2022_normfit_1850_2022_by_return_period_QN_NN_dec_2022.csv'), index_col=0)
...
colors=['blue', 'none', 'fuchsia']
for bar, color in zip(barlist, colors):
    bar.set_color(color)
plt.scatter(center_cf[[0,2]], y_pos[[0,2]], s=200, marker='|', color='k', zorder=4)
...
for bar, color in zip(barlist, colors):
    bar.set_color(color)
plt.scatter(center_ac[[0,2]], y_pos[[0,2]], s=200, marker='|', color='k', zorder=4)
plt.yticks(y_pos, model_names)
ax.invert_yaxis()  # labels read top-to-bottom
...
for bar, color in zip(barlist, colors):
    bar.set_color(color)
plt.scatter(center_rr[[0,2]], y_pos[[0,2]], s=200, marker='|', color='k', zorder=4)
plt.yticks(y_pos, model_names)
ax.invert_yaxis()  # labels read top-to-bottom
...
plt.tight_layout()
plt.savefig('../../../megafires_data/png/synthesis_plot_metrics_normfit_MLE_QN_NN_dec_2022.png', dpi=300, bbox_inches = 'tight', pad_inches = 0)
plt.show()
