...
ri_full = ut.get_regional_index()

ri = ri_full.sel(time=slice('1960', '2021'))
lens2_gmst = lens2_gmst_full.sel(time=slice('1960', '2021'))
lens2_tmax = lens2_tmax_full.sel(time=slice('1960', '2021'))

lens2_tmax_anom = lens2_tmax - lens2_tmax.mean('time')
lens2_tmax_corrected = lens2_tmax_anom + ri.mean('time')
...
for i in range(nboot):
    ri_i, sm_i = bootstrap(lens2_tmax_arr, lens2_gmst_arr)
    xopt_i = pmath.mle_norm_2d_fast(ri_i, sm_i, [29.57, 1.15, 0.88])
...
filepath = '../../../megafires_data/output/MLE_tasmax_jan_LENS2_GMST_'+str(nboot)+'_normal_validation_RI.nc'
