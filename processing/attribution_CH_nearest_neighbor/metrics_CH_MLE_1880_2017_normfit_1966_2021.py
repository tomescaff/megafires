...
qnf = stns.get_CH_tmax_jan()

sm = smf.sel(time=slice('1966','2021'))
qn = qnf.sel(time=slice('1966','2021'))

sm_no2017 = sm.where(sm.time.dt.year != 2017, drop=True)
qn_n02017 = qn.where(qn.time.dt.year != 2017, drop=True)

xopt = pmath.mle_norm_2d(qn_n02017.values, sm_no2017.values, [29.44, 0.81, 1.41])
...
# bootstrap MLE
nboot = 1000
filepath = '../../../megafires_data/output/MLE_tasmax_jan_CH_GMST_'+str(nboot)+'_normal_validation.nc'
...
df.to_csv(f'../../../megafires_data/output/metrics_CH_MLE_{cf_year}_{ac_year}_normfit_1966_2021.csv')